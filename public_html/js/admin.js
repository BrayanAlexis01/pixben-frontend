...
let datosGaleriaEdicion = {imagenes: [], imagenesPorVariante: {}};
let productosAdmin = [];
let paginaProductosAdmin = 1;
const PRODUCTOS_ADMIN_POR_PAGINA = 20;
const productosSeleccionadosAdmin = new Set();
const CLAVE_MANTENER_RENDER = "pixben_admin_mantener_render_activo";
...
function configurarEventos() {
    ...
    document.getElementById("listaColoresAdmin").addEventListener("change", evento => {
        const input = evento.target.closest("[data-color-imagenes]");
        if (input) actualizarPrevisualizacionColor(input.closest(".fila-color-admin"));
    });
...
function mantenerRenderActivo() {
    return sessionStorage.getItem(CLAVE_MANTENER_RENDER) === "1";
}

function actualizarEstadoRender(estado, detalle = "") {
    const punto = document.getElementById("puntoEstadoRender");
    const texto = document.getElementById("textoEstadoRender");
    const detalleElemento = document.getElementById("detalleEstadoRender");
    if (!punto || !texto || !detalleElemento) return;

    punto.className = `punto-estado-render ${estado}`;
    const etiquetas = {
...
function actualizarBotonMantenerRender(activo) {
    const boton = document.getElementById("btnMantenerRender");
    if (!boton) return;
...
async function comprobarEstadoRender({silencioso = false} = {}) {
    if (comprobandoRender) return;
    comprobandoRender = true;
    if (!silencioso) actualizarEstadoRender("comprobando", "Consultando /api/health...");
...
function sincronizarMantenerRender() {
    if (temporizadorMantenerRender) clearInterval(temporizadorMantenerRender);
    temporizadorMantenerRender = null;
    const activo = mantenerRenderActivo();
    actualizarBotonMantenerRender(activo);
    if (!activo) return;
    comprobarEstadoRender();
    temporizadorMantenerRender = setInterval(() => comprobarEstadoRender({silencioso: true}), INTERVALO_MANTENER_RENDER_MS);
}
...
function agregarFilaColor(datos = {}) {
    ...
    fila.dataset.imagenesActuales = "0";
    fila.dataset.indiceLegacy = Number.isInteger(Number(datos.imagenIndice)) ? String(Number(datos.imagenIndice)) : String(lista.children.length);
    fila.innerHTML = `
        <input type="hidden" data-color-clave value="${escaparAtributo(datos.clave || "")}">
        <span class="orden-color-admin"></span>
        <label>Nombre<input type="text" data-color-nombre maxlength="60" placeholder="Ej. Negro" value="${escaparAtributo(datos.nombre || "")}"></label>
        ...
        <div class="galeria-variante-admin">
            <div class="galeria-variante-cabecera">
                <div><strong>Fotos de este color</strong><small>Sube entre 1 y 7 fotos: frente, espalda y detalles. Al elegir este color en la tienda, solo se mostrarán estas imágenes.</small></div>
                <span data-color-contador>0 nuevas</span>
            </div>
            <input type="file" data-color-imagenes accept="image/webp,image/jpeg,image/png" multiple>
            <div class="previsualizacion-color-admin" data-color-preview></div>
            <div class="imagenes-color-actuales" data-color-actuales></div>
        </div>`;
    lista.appendChild(fila);
    reindexarColoresAdmin();
    actualizarStockColoresAdmin();
    if (!datos.nombre) fila.querySelector("[data-color-nombre]")?.focus();
}

function reindexarColoresAdmin() {
    document.querySelectorAll("#listaColoresAdmin .fila-color-admin").forEach((fila, indice) => {
        fila.querySelector(".orden-color-admin").textContent = `Color ${indice + 1}`;
    });
}
...
function obtenerColoresAdmin() {
    const colores = Array.from(document.querySelectorAll("#listaColoresAdmin .fila-color-admin")).map((fila, indice) => ({
        clave: fila.querySelector("[data-color-clave]").value.trim() || null,
...
function actualizarStockColoresAdmin() {
    ...
    const galeriaGeneral = document.getElementById("grupoGaleriaGeneral");
    if (!filas.length) {
        stock.readOnly = false;
        ayuda.textContent = "Se usa cuando el producto no tiene variantes de color.";
        if (galeriaGeneral) galeriaGeneral.hidden = false;
        return;
    }
    ...
    if (galeriaGeneral) galeriaGeneral.hidden = true;
}

function archivosColorFila(fila) {
    return Array.from(fila?.querySelector("[data-color-imagenes]")?.files || []);
}
...
function actualizarPrevisualizacionColor(fila) {
    if (!fila) return;
    const archivos = archivosColorFila(fila);
    if (archivos.length > MAXIMO_IMAGENES) {
        fila.querySelector("[data-color-imagenes]").value = "";
        alert("Solo puedes seleccionar hasta 7 imágenes por color.");
        return actualizarPrevisualizacionColor(fila);
    }
    try { archivos.forEach(validarArchivoImagen); }
    catch (error) {
        fila.querySelector("[data-color-imagenes]").value = "";
        alert(error.message);
        return actualizarPrevisualizacionColor(fila);
    }
    fila.querySelector("[data-color-contador]").textContent = archivos.length ? `${archivos.length} nueva${archivos.length === 1 ? "" : "s"}` : "Sin cambios";
    const preview = fila.querySelector("[data-color-preview]");
    preview.innerHTML = "";
    archivos.forEach((archivo, indice) => {
        ...
        pie.textContent = indice === 0 ? "Portada del color" : `Foto ${indice + 1}`;
        figura.append(imagen, pie);
        preview.appendChild(figura);
    });
}

function validarGaleriasColores() {
    const filas = Array.from(document.querySelectorAll("#listaColoresAdmin .fila-color-admin"));
    filas.forEach(fila => {
        const nombre = fila.querySelector("[data-color-nombre]").value.trim() || "este color";
        const archivos = archivosColorFila(fila);
        if (archivos.length > MAXIMO_IMAGENES) throw new Error(`Solo puedes subir hasta 7 imágenes para ${nombre}.`);
        archivos.forEach(validarArchivoImagen);
        const existentes = Math.max(0, Number(fila.dataset.imagenesActuales || 0));
        if (!archivos.length && existentes === 0) {
            throw new Error(`Agrega al menos una imagen para el color ${nombre}.`);
        }
    });
}
...
function validarImagenes(archivos, obligatorio) {
    if (archivos.length > MAXIMO_IMAGENES) throw new Error("Solo puedes seleccionar hasta 7 imágenes.");
    if (obligatorio && archivos.length === 0 && cantidadImagenesActuales === 0) throw new Error("Selecciona al menos una imagen para la galería general.");
    archivos.forEach(validarArchivoImagen);
}
...
async function subirGaleriaVariante(productoId, claveColor, archivos) {
    if (!archivos.length || !claveColor) return;
    const formData = new FormData();
    archivos.forEach(archivo => formData.append("archivos", archivo));
    const respuesta = await fetchConSesion(`${API_URL}/imagenes/${productoId}/variantes/${encodeURIComponent(claveColor)}`, {method: "POST", body: formData});
    if (!respuesta.ok) throw new Error(await obtenerMensajeError(respuesta));
}
...
async function guardarProducto(evento) {
    ...
    try {
        ...
        validarImagenes(archivos, colores.length === 0 && !id);
        if (colores.length) validarGaleriasColores();
        ...
        const skuProducto = document.getElementById("sku").value.trim().toUpperCase();
        const duplicadoSku = skuProducto && productosAdmin.find(p =>
            normalizarTexto(p.sku) === normalizarTexto(skuProducto) && String(p.id) !== String(id)
        );
        if (duplicadoSku) {
            throw new Error(`El SKU ${skuProducto} ya pertenece a ${duplicadoSku.nombre}.`);
        }
        const duplicadoNombre = productosAdmin.find(p =>
            normalizarTexto(p.nombre) === normalizarTexto(nombreProducto) &&
            normalizarTexto(p.categoria) === normalizarTexto(categoriaSeleccionada) &&
            String(p.id) !== String(id)
        );
        if (duplicadoNombre && !confirm(`Ya existe "${duplicadoNombre.nombre}" en esta categoría (ID ${duplicadoNombre.id}). ¿Deseas guardar otro producto de todas formas?`)) {
            return;
        }

        const producto = {
            nombre: nombreProducto,
            sku: skuProducto || null,
...
async function cargarProductos() {
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td colspan="12" class="error-admin">${escaparHtml(error.message)}</td></tr>`;
    }
}
...
function obtenerProductosFiltradosAdmin() {
    ...
    const lista = productosAdmin.filter(p => {
        const valores = {
            ...
            sku: normalizarTexto(p.sku),
            id: String(p.id ?? "")
        };
        const base = campo === "todo" ? normalizarTexto(`${p.id} ${p.sku || ""} ${p.nombre} ${p.categoria} ${p.descripcion}`) : valores[campo] || "";
...
function renderizarProductosAdmin() {
    ...
    if (!pagina.length) {
        tbody.innerHTML = '<tr><td colspan="12" class="estado-carga">No hay productos que coincidan con los filtros.</td></tr>';
    } else {
        tbody.innerHTML = pagina.map(p => {
            ...
            return `<tr>
                ...
                <td>${p.id}</td><td>${escaparHtml(p.sku || "—")}</td><td><b>${escaparHtml(p.nombre)}</b></td><td>${escaparHtml(p.categoria || "Sin categoría")}</td><td>S/ ${Number(p.precio || 0).toFixed(2)}</td><td>${Number(p.stock || 0)}</td><td>${escaparHtml(detalleColores)}</td><td>${escaparHtml(tallas)}</td><td>${p.personalizable ? '<span class="chip-si">Sí</span>' : '<span class="chip-no">No</span>'}</td>
...
async function abrirVistaPreviaProductoAdmin(id) {
    ...
    try{const r=await fetchConSesion(`${API_URL}/imagenes/${id}`);if(r.ok){const g=await r.json();(g.imagenes||[]).forEach(img=>{const u=obtenerUrlImagen(img);if(!urls.includes(u))urls.push(u);});Object.values(g.imagenesPorVariante||{}).flat().forEach(img=>{const u=obtenerUrlImagen(img);if(!urls.includes(u))urls.push(u);});}}catch{}
...
async function mostrarImagenesActuales(productoId, principal) {
    ...
    let galeria = {imagenes: [], imagenesPorVariante: {}};
    try {
        const respuesta = await fetchConSesion(`${API_URL}/imagenes/${productoId}`, {cache: "no-store"});
        if (respuesta.ok) {
            galeria = await respuesta.json();
    ...
    datosGaleriaEdicion = {
        imagenes: Array.isArray(galeria.imagenes) ? galeria.imagenes : [],
        imagenesPorVariante: galeria.imagenesPorVariante && typeof galeria.imagenesPorVariante === "object" ? galeria.imagenesPorVariante : {}
    };
    cantidadImagenesActuales = urls.length;
    const contenedor = document.getElementById("imagenesActuales");
    contenedor.innerHTML = urls.map((url, indice) => `<figure><img src="${escaparAtributo(url)}" alt="Imagen general ${indice + 1}"><figcaption>${indice === 0 ? "Portada" : `General ${indice + 1}`}</figcaption></figure>`).join("");

    const filas = Array.from(document.querySelectorAll("#listaColoresAdmin .fila-color-admin"));
    filas.forEach((fila, indiceFila) => {
        const clave = fila.querySelector("[data-color-clave]")?.value.trim().toLowerCase();
        let propias = clave ? datosGaleriaEdicion.imagenesPorVariante[clave] : null;
        propias = Array.isArray(propias) ? propias.map(obtenerUrlImagen) : [];
        // Productos antiguos: hasta que el administrador suba la galería nueva, mostramos la imagen vinculada por índice.
        if (!propias.length && urls.length) {
            const indiceLegacy = Math.max(0, Number(fila.dataset.indiceLegacy ?? indiceFila));
            const heredada = urls[indiceLegacy] || urls[0];
            if (heredada) propias = [heredada];
        }
        fila.dataset.imagenesActuales = String(propias.length);
        const actuales = fila.querySelector("[data-color-actuales]");
        actuales.innerHTML = propias.length
                ? `<p>Galería actual</p><div>${propias.map((url, i) => `<figure><img src="${escaparAtributo(url)}" alt="Foto actual ${i + 1}"><figcaption>${i === 0 ? "Portada" : `Foto ${i + 1}`}</figcaption></figure>`).join("")}</div>`
                : '<p class="sin-galeria-color">Aún no hay fotos para este color.</p>';
        const contador = fila.querySelector("[data-color-contador]");
        if (contador) contador.textContent = propias.length ? `${propias.length} actual${propias.length === 1 ? "" : "es"}` : "Sin fotos";
    });
}
...
function limpiarFormulario() {
    ...
    datosGaleriaEdicion = {imagenes: [], imagenesPorVariante: {}};
