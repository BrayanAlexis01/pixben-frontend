...
async function obtenerGaleriaProducto(producto) {
    ...
    if (!Number.isFinite(idProducto)) return [principal];
    if (!CACHE_GALERIAS_PRODUCTO.has(idProducto)) {
        CACHE_GALERIAS_PRODUCTO.set(idProducto, (async () => {
            const urls = [principal];
            try {
                const respuesta = await fetch(`${API_URL}/imagenes/${idProducto}`);
                if (respuesta.ok) {
                    const datos = await respuesta.json();
                    (Array.isArray(datos?.imagenes) ? datos.imagenes : []).forEach(imagen => {
                        const url = obtenerUrlImagen(imagen);
                        if (url && !urls.includes(url)) urls.push(url);
                    });
                }
            } catch (error) {
                console.warn("No se pudo cargar la imagen de la variante:", error);
            }
            return urls;
        })());
    }
...
async function obtenerImagenVarianteProducto(producto, nombreColor) {
    ...
    if (!variante) return obtenerUrlImagen(producto?.imagen);
    const galeria = await obtenerGaleriaProducto(producto);
    return galeria[variante.imagenIndice] || galeria[0] || IMAGEN_FALLBACK;
}
