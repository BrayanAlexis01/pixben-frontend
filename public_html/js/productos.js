...
async function cargarProductosCatalogo() {
    mostrarEsqueletoCatalogo();
    mostrarEstado(
        '<span class="spinner-catalogo" aria-hidden="true"></span>' +
        '<span><b>Cargando productos...</b><small> El catálogo se administra directamente desde el panel.</small></span>',
        false,
        true
    );
...
function mostrarEsqueletoCatalogo() {
    if (!elementosCatalogo.grid) return;
    elementosCatalogo.grid.innerHTML = Array.from({length: 8}, () => `
        <article class="producto producto-esqueleto" aria-hidden="true">
            <div class="producto-imagen-contenedor esqueleto-bloque"></div>
            <div class="producto-info">
                <span class="esqueleto-linea corta"></span>
                <span class="esqueleto-linea"></span>
                <span class="esqueleto-linea precio"></span>
            </div>
        </article>`).join("");
}
    ...
    elementosCatalogo.grid.innerHTML = pagina.map(producto => {
        ...
        return `
            <article class="producto" tabindex="0" role="link"
                     data-producto-id="${identificador}"
                ...
                <div class="producto-imagen-contenedor">
                    ...
                    <img src="${escaparHtmlSeguro(obtenerUrlImagen(producto.imagen))}"
                         alt="${escaparHtmlSeguro(producto.nombre)}"
                         loading="lazy"
...
function mostrarEstado(mensaje, esError = false, permitirHtml = false) {
    if (!elementosCatalogo.estado) {
        return;
    }
    if (permitirHtml) elementosCatalogo.estado.innerHTML = mensaje;
    else elementosCatalogo.estado.textContent = mensaje;
    elementosCatalogo.estado.classList.toggle("error", esError);
    elementosCatalogo.estado.classList.toggle("cargando", permitirHtml && Boolean(mensaje));
